...
#adding new game over logic -- if the snake touches thw walls of the display screen, the game gets over
#to make the game a little more challenging, after a threshold time of game not getting over, the difficulty level of game increases as speed of snake increases -- by reducing sleep time to less than 0.2
...
#--
INITIAL_SLEEP_TIME = 0.2  # Initial sleep time to control the snake's starting speed
...
class Game:
    ...
    #function to play bgm in the game
...
#--               
            ...
            if not (0 <= self.snake.x[0] <= GAME_WIDTH-SIZE and 0 <= self.snake.y[0] <= GAME_LENGTH-SIZE):
                    self.play_sound('crash')
    ...
    def run(self):
            ...
            pause = False
#--
            sleep_time = INITIAL_SLEEP_TIME  # Set the initial sleep time
            ...
            #importing local global variables for this code using pygame.locals
                ...
                for event in pygame.event.get():
                    if event.type == KEYDOWN: 
                        ...
                        if not pause:
                            if event.key == K_UP or event.key == K_w:  #incremented Y-coordinate
                                self.snake.move_up()
                ...
                try:
                    if not pause:
                        ...
                        if elapsed_time > 30: 
                            sleep_time = max(0.04, sleep_time - 0.04)  # Decrease sleep time but not below 0.05
                ...
                # time.sleep(0.2) #to create a delay in snake movement -- without it the game speed will be unplayable, CPU Usage will me higher and Game enjoyment will get reduced
#--
                time.sleep(sleep_time)  # Use the updated sleep time to control game speed
